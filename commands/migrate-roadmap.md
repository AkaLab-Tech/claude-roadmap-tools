...
2. **Pull all three buckets.**
   ...
   - **Verify each `listTasks` call returned the full result set** per its backend's pagination/limit mandate (see the `listTasks(bucket)` sections under [Operations](../skills/roadmap-tracking-flow/SKILL.md#operations-filesbackend) in SKILL.md — cursor pagination to `hasNextPage: false` for `linear`/`github-project`, `gh issue list --limit 1000` re-checked for `github-issues`). If completeness cannot be confirmed for any bucket, **stop before step 3** — a migration that silently drops items past a page/limit boundary breaks the losslessness this step promises.
...
3. **Show the full reconstruction plan before writing anything.**
   ...
   - Note explicitly that, if the repo root is inside a git working tree, **all five** `.git/info/exclude` entries the forward legs may have added (`ROADMAP.md`, `IN_PROGRESS.md`, `HISTORY.md`, `roadmap/`, `.plan/`) will be **removed unconditionally** — regardless of which of those artefacts step 5d.4 actually reconstructed — and the reconstructed artefacts will be **re-tracked** (`git add`, not committed) — see step 5d.5.
...
5. **Revert the forward legs' local git tracking changes, and re-track the reconstructed files.** This must run **before** the `.roadmap.json` removal (next step): if this step fails, `.roadmap.json` is still present and the migration is cleanly re-runnable — the same "leave `.roadmap.json` in place on failure" property the partial-failure guard (step 5d.7) already gives steps 5d.2 and 5d.4.
   - **If the repo root is inside a git working tree** (mirroring the forward legs' "If the repo root is inside a git working tree" conditional in 5b.7 / 5c.7 / 5e.7). If it is not, this whole step is a no-op — never error just because the reconstruction is happening outside a git working tree.
     - **Remove from `.git/info/exclude`** all five of the lines the forward legs may have added — `ROADMAP.md`, `IN_PROGRESS.md`, `HISTORY.md`, `roadmap/`, `.plan/` — **unconditionally, regardless of which artefacts step 5d.4 actually reconstructed.** The forward legs append all five together, unconditionally, whenever `offlineMirror` is `true` (the `true` branch of 5b.7 / 5c.7 / 5e.7), so a partial reconstruction (e.g. no task carried an `atelier:plan` section, so `.plan/` was never written) must still fully undo the exclude. This matters concretely for `.plan/`: under the `files` backend `.plan/<id>.md` is committed repo content, not a mirror artefact, so leaving its exclude entry behind after authority flips to files would make the next plan write silently git-ignored. Be surgical: remove only those exact atelier-added lines, never rewrite, reorder, or truncate any other line in the file — any unrelated operator-added entries in `.git/info/exclude` must survive untouched. Removing an entry for a path that was not reconstructed (and so does not exist on disk) is harmless. If the file is absent, or none of the entries are present, this is a clean no-op. **Never touch the committed `.gitignore`** — the forward legs never wrote to it, so there is nothing to undo there.
